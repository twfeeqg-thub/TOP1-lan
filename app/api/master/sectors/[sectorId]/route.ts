  ...
  try {
    ...
    if (typeof body === 'object' && body !== null && 'is_active' in body && typeof body.is_active === 'boolean') {
      const data = await withMasterTx(actor, async (tx) => {
        const exists = await tx.query('SELECT id FROM core.sectors WHERE id = $1;', [sectorId])
        if (exists.rows.length === 0) {
          throw new Error('SECTOR_NOT_FOUND')
        }
        await tx.query('UPDATE core.sectors SET is_active = $1 WHERE id = $2;', [body.is_active, sectorId])
        const updated = await tx.query('SELECT full_data FROM core.sectors WHERE id = $1;', [sectorId])
        return {
          data: updated.rows[0].full_data,
          audit: {
    ...
    const fullData = body as SectorData
    const data = await withMasterTx(actor, async (tx) => {
      const exists = await tx.query('SELECT id FROM core.sectors WHERE id = $1;', [sectorId])
      if (exists.rows.length === 0) {
        throw new Error('SECTOR_NOT_FOUND')
      ...
      await tx.query(
        ...
        [JSON.stringify(fullData), sectorId]
      )
      return {
        data: fullData,
        audit: {
    ...
    if (err instanceof Error && err.message === 'SECTOR_NOT_FOUND') {
      return NextResponse.json({ error: 'Sector not found' }, { status: 404 })
    }
...
export async function DELETE(
  ...
  try {
    ...
    await withMasterTx(actor, async (tx) => {
      const result = await tx.query('DELETE FROM core.sectors WHERE id = $1 RETURNING id;', [sectorId])
      if (result.rows.length === 0) {
        throw new Error('SECTOR_NOT_FOUND')
      }
      return {
