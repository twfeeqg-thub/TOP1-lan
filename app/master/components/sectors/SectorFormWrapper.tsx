  ...
  return (
    <div className="flex gap-6">
      ...
      <div className="flex-1 min-w-0">
        ...
        >
          {activeTab === 'hero' && (
            <HeroSectionForm data={data.hero} onChange={(v) => updateSection('hero', v)} />
          )}
          {activeTab === 'projects' && (
            <ProjectsSectionForm data={data.projects} onChange={(v) => updateSection('projects', v)} />
          )}
          {activeTab === 'about' && (
            <AboutSectionForm data={data.about} onChange={(v) => updateSection('about', v)} />
          )}
          {activeTab === 'testimonials' && (
            <TestimonialsSectionForm data={data.testimonials} onChange={(v) => updateSection('testimonials', v)} />
          )}
          {activeTab === 'faq' && (
            <FAQSectionForm data={data.faqs} onChange={(v) => updateSection('faqs', v)} />
          )}
          {activeTab === 'partners' && (
            <PartnersSectionForm data={data.partners} onChange={(v) => updateSection('partners', v)} />
          )}
          {activeTab === 'legal' && (
            <LegalFooterSectionForm data={data.legal_footer} onChange={(v) => updateSection('legal_footer', v)} />
          )}
        </motion.div>
      </div>
    </div>
