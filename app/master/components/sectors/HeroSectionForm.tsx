...
export function HeroSectionForm({ data, onChange }: HeroSectionFormProps) {
  const cta_primary = data?.cta_primary ?? { text: '', href: '' }
  const cta_secondary = data?.cta_secondary ?? { text: '', href: '' }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <input
            type="text"
            value={cta_primary.text}
            onChange={(e) => update('cta_primary', { ...cta_primary, text: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-[var(--sidebar-hover-bg)] border border-[var(--card-border)] text-[var(--text-main)] text-sm focus:outline-none focus:border-[var(--primary)] transition-colors"
          />
          <label className="block text-sm font-medium text-[var(--text-main)]">زر CTA الأساسي — الرابط</label>
          <input
            type="text"
            value={cta_primary.href}
            onChange={(e) => update('cta_primary', { ...cta_primary, href: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-[var(--sidebar-hover-bg)] border border-[var(--card-border)] text-[var(--text-main)] text-sm focus:outline-none focus:border-[var(--primary)] transition-colors"
          />
        </div>
        <div className="space-y-2">
          ...
          <input
            type="text"
            value={cta_secondary.text}
            onChange={(e) => update('cta_secondary', { ...cta_secondary, text: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-[var(--sidebar-hover-bg)] border border-[var(--card-border)] text-[var(--text-main)] text-sm focus:outline-none focus:border-[var(--primary)] transition-colors"
          />
          <label className="block text-sm font-medium text-[var(--text-main)]">زر CTA الثانوي — الرابط</label>
          <input
            type="text"
            value={cta_secondary.href}
            onChange={(e) => update('cta_secondary', { ...cta_secondary, href: e.target.value })}
            className="w-full px-4 py-2.5 rounded-xl bg-[var(--sidebar-hover-bg)] border border-[var(--card-border)] text-[var(--text-main)] text-sm focus:outline-none focus:border-[var(--primary)] transition-colors"
          />
        </div>
      </div>
